...
<?php 
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        $req_date = $_POST['req_date'];
         ...
         $searchProduct = "SELECT * FROM tbl_request ";
         $statement = $pdo->query($searchProduct);
         $productInfo = $statement->fetchAll(PDO::FETCH_ASSOC);
         $countProd = $statement->rowCount();
         
        //  if($countProd > 0){
        //      echo "<script>alert('You Already Request Today!');
        //      window.location.href='../../user/request.php';
        //      </script>";
        //  }
        //  else{
 
            $sql = 'INSERT INTO tbl_request(tracking_id,req_date, fullname, request_type, purpose, date_open, date_close, get_date, payment_method,reference_no,amount,date_paid, payment_status, request_status,username) VALUES (:tracking_id,:req_date ,:fullname, :request_type, :purpose, :date_open, :date_close, :get_date, :payment_method,:reference_no,:amount,:date_paid, :payment_status, :request_status,:username)';
            ...
            if($_POST['payment_method'] == 'Pick Up' || $_POST['payment_method'] == 'None' )
            {
