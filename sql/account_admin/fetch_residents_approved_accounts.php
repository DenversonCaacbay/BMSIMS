...
<script>
        $(document).ready(function () {
            ...
            $('.grantbtn').on('click', function () {
                ...
                console.log(data);
                $('#acc_id6').val(data[0]);
                $('#admin_power6').val(data[1]);
                $('#username6').val(data[2]);
                $('#firstname6').val(data[3]);
...
$query = "
SELECT * FROM resident_accounts WHERE access='Approved'
";
...
<table class="table sticky">
  <tr>
    ...
    <th class="text-center">Ban Account</th>
...
if($total_data > 0)
{
  foreach($result as $row)
  {
    ...
    <tr>
      ...
      <td class="text-center">'.date("F d, Y- l [g:i:s A]", strtotime($row["updated_at"])).'</td>
      <td class="text-center">'.$row["fname"].'</td>
      <td class="text-center">'.$row["mname"].'</td>
      <td class="text-center">'.$row["lname"].'</td>
      <td class="text-center" hidden>'."<img data-id=".$row['res_id']." class='userinfo idbtn' src='data:image/jpeg;base64,".base64_encode($row['validation'])."'width=50px height=50px alt='IMAGE' />".'</td>
    ...
    if($row["access"] == "Not Yet Approve"){
      $output .= '<td>'.'<button type="button" style="margin-left:5px;" class="btn btn-custom grantbtn">Grant Access</button>'.'</td></tr>';
    }
    else if($row["access"] == "Banned"){
      $output .= '<td>'.'<button type="button" style="margin-left:5px;" class="btn btn-custom grantbtn">Grant Access</button>'.'</td></tr>';
    }
    else {
      $output .= '
      <td class="text-center">'.'<button  class="btn btn-custom"><a class="text-white" style="text-decoration:none;" href="https://mail.google.com/mail/?view=cm&fs=1&to='.$row["email"].'&su=Welcome Resident&body=Click the Link to Login: https://portfoliopromax.com/bmsims/user/user_index.php" target="_blank"><i class="fas fa-paper-plane text-white"></i>Send Mail</a></button>'.'</td>
      </tr>';
    };
