...
if($_POST['query'] != '')
{
  ...
  OR admin_power LIKE "%'.str_replace(' ', '%', $_POST['query']).'%" 
  ';
}
...
if($total_data > 0)
{
  foreach($result as $row)
  {
    ...
    <tr>
      ...
      <td hidden>'.$row["place_of_birth"].'</td>
      <td hidden>'.$row["bdate"].'</td>
      <td hidden>'.$row["civil_status"].'</td>
      <td hidden>'.$row["address"].'</td>
      <td hidden>'.$row["purok"].'</td>
      <td hidden>'.$row["phone"].'</td>
      <td>'."<img data-id=".$row['acc_id']." class='userinfo idbtn' src='data:image/jpeg;base64,".base64_encode($row['image'])."'width=50px height=50px alt='IMAGE' />".'</td>
      <td hidden>'.$row["password"].'</td>
      <td>'.'<button type="button" style="width:100%;" class="btn viewbtn"><i class="fas fa-user-edit"></i></button>'.'</td>
    ...
    if($row["admin_power"] == "Not Approved"){
      $output .= '<td>'.'<button type="button" style="margin-left:5px;" class="btn btn-custom grantbtn">Grant Access</button>'.'</td></tr>';
    }
    else if($row["admin_power"] == "Banned"){
      $output .= '<td>'.'<button type="button" style="margin-left:5px;" class="btn btn-custom grantbtn">Grant Access</button>'.'</td></tr>';
    }
    else {
      $output .= '
      <td>'.'<a href="https://mail.google.com/mail/?view=cm&fs=1&to='.$row["email"].'&su=Click the Link to Login&body=http://localhost/bmsims/user/user_index.php" target="_blank">Send Mail</a>'.'</td>
      </tr>';
    };
